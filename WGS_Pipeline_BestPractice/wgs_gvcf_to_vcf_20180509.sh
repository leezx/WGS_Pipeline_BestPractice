...
time $gatk CombineGVCFs \
	...
	${sample_gvcfs} \
	-O $outdir/poplation/${outname}.HC.g.vcf.gz && echo "** ${outname}.HC.g.vcf.gz done ** " && \
time $gatk GenotypeGVCFs \
	-R $reference/Homo_sapiens_assembly38.fasta \
	-V $outdir/poplation/${outname}.HC.g.vcf.gz \
	-O $outdir/poplation/${outname}.HC.vcf.gz && echo "** ${outname}.HC.vcf.gz done ** "
...
#         -R $reference/Homo_sapiens_assembly38.fasta \
#         ${sample_gvcfs} \
#         -O $outdir/poplation/${outname}.HC.${i}.g.vcf.gz && echo "** ${outname}.HC.${i}.g.vcf.gz done ** " && \
#     time $gatk GenotypeGVCFs \
#         -R $reference/Homo_sapiens_assembly38.fasta \
#         -V $outdir/poplation/${outname}.HC.${i}.g.vcf.gz \
#         -O $outdir/poplation/${outname}.HC.${i}.vcf.gz && echo "** ${outname}.HC.${i}.vcf.gz done ** " &
# done && wait
# merge_vcfs=""
# for i in ${chrom[@]}; do
#     merge_vcfs=${merge_vcfs}"-I $outdir/poplation/${outname}.HC.${i}.vcf.gz \\"\n
# done && time $gatk MergeVcfs ${merge_vcfs} -O $outdir/poplation/${outname}.HC.vcf.gz && echo "** MergeVcfs done **"


## VQSR, 由于评价SNP和Indel质量高低的标准是不同的，因此，需要分SNP和Indel这两种不同的模式，分别进行质控

# ## 这一步不是必须的，仅仅是为了提高速度，如果你的样本数目不多比如（500以下），那么可以选择忽略.如果你的样本很多（>500）,
# ## 考虑到处理速度，我们可以用“MakeSitesOnlyVcf”只把sites的信息提取出来，忽略VCF后面的个体信息，从而节省IO时间。
# ## 如果提取了Only sites，那么注意下面VQSR中的输入文件要替换为这里的${outname}.HC.sites.vcf.gz
# time $gatk VariantRecalibrator \
#     -I $outdir/poplation/${outname}.HC.vcf.gz \
#     -O $outdir/poplation/${outname}.HC.sites.vcf.gz && echo "** Only sites done **"
...
    -resource omin,known=false,training=true,truth=false,prior=12.0:$GATK_bundle/1000G_omni2.5.hg38.vcf \
...
    --rscript-file $outdir/poplation/${outname}.HC.snps.plots.R \
...
    --tranches-file $outdir/poplation/${outname}.HC.snps.tranches \
    --recal-file $outdir/poplation/${outname}.HC.snps.recal \
    -mode SNP \
...
time $gatk VariantRecalibrator \
 ...
    -mode INDEL \
    --max-gaussians 6 \
    --rscript-file $outdir/poplation/${outname}.HC.snps.indels.plots.R \
...
    --tranches-file $outdir/poplation/${outname}.HC.snps.indels.tranches \
    --recal-file $outdir/poplation/${outname}.HC.snps.indels.recal \
    -mode INDEL \
...
# rm -f $outdir/poplation/${outname}.HC.snps.VQSR.vcf.gz 
# $outdir/poplation/${outname}.HC.*.recal
